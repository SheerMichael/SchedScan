...
export default function Scanner() {
  ...
  const handleCameraCapture = async () => {
    ...
    try {
      ...
      if (!permissionResult.granted) { Alert.alert('Permission Required', 'Please allow access to your camera'); return; }
  ...
  const uploadFile = async (file: any, uploadType: string) => {
    ...
    try {
      ...
      if (user?.id) await scheduleStorageService.recordUpload(user.id);
